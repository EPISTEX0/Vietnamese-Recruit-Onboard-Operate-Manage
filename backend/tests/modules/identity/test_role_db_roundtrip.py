...
# test file: backend/tests/modules/identity/test_role_db_roundtrip.py
...
def postgres_async_url() -> Iterator[str]:
    ...
    The schema is built by ``alembic upgrade head`` rather than
    ``SQLModel.metadata.create_all`` so the column these tests read through is
    the one production actually has, including the widening done by 084.
    """
    docker = pytest.importorskip("docker")
    postgres_container = pytest.importorskip("testcontainers.postgres")

    if not _docker_available(docker):
        pytest.skip("Docker is not available for the role round-trip tests")
