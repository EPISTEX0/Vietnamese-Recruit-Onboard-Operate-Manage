...
1. **System Admin (Deployer / Quản trị hệ thống)**: Người chịu trách nhiệm First-Run Setup, cấu hình hạ tầng, kết nối Google Workspace OAuth Client ID/Secret, quản lý LLM Providers/API Keys, Email Whitelist và System Audit Logs.
...
- **`/api/system-admin/*`**: Gated bởi `require_system_admin`. Bao gồm: AI Config (`/ai-config/*`), OAuth Config (`/oauth/config`), Whitelist (`/whitelist`), System Audit Logs (`/audit-logs`), User/Role Management (`/users`), Org Domains (`/organization/domains`), Assistant Tools (`/assistant-tools`), Runtime Health (`/runtime/health`).
