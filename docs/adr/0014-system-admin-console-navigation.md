...
> **Con số "bảy đề mục" đúng tại thời điểm ADR này, và hết đúng từ #307.** Đề mục thứ tám — Google OAuth, `/settings/oauth` — vào nhóm "Người dùng & Truy cập" và đứng đầu nhóm đó, vì nó quyết định ai đăng nhập được, còn ba đề mục dưới nó chỉ có nghĩa sau khi có người đăng nhập được. Cấu trúc mà ADR này quyết định — route thật cho từng đề mục, ba nhóm, base path `/settings`, trang chủ riêng — **không đổi**; chỉ số đếm đổi. Xem thêm đính chính ở gạch đầu dòng "Task Cấu hình Google OAuth…" trong Consequences.
...
- **Task "Cấu hình Google OAuth" của Quick-Start Guide không có đích điều hướng, và đó là sự thật về hệ thống chứ không phải thiếu sót của trang chủ.**
  ...
  >
  > **Ticket đó là #307 và đã xong: task này giờ trỏ tới `/settings/oauth`.** Dự đoán ở đoạn trên đúng nguyên văn — wiring là đúng **một dòng** trong `TASK_ACTIONS` của `lib/system-admin/setup-guide.ts`, `page.tsx` không sửa dòng nào, vì `SetupTaskRow` vốn gate theo `task.action` có hay không chứ không theo id của task. Nhánh `action: null` vẫn ở lại: nó là câu trả lời của module cho task tiếp theo hạ cánh trước đề mục của nó, và giờ được test qua route map inject vào `buildSetupGuide` — sau #307 không còn task sống nào đi qua nhánh ấy nữa. Kèm theo: `taskGoogleOAuthGuidance` trong `messages/*.json` trở thành chuỗi không tới được, và câu chữ của nó ("Console chưa có màn hình cho mục này") không còn đúng — cố ý giữ nguyên thay vì gỡ, vì gỡ là quyết định về cơ chế guidance của #302 chứ không phải phần wiring của #307.
  >
  > **Đã thay đổi.** Mệnh đề "nhánh `action: null` vẫn ở lại" về sau bị đảo — xem [ADR 0015](./0015-quick-start-guide-task-always-has-destination.md), thứ ticket được hoãn tới ở đoạn trên cuối cùng quyết định. `SetupTaskView.action` không còn nullable; invariant từ đó là mọi task trong `SETUP_TASK_IDS` đều có đích đến, và `taskGoogleOAuthGuidance` đã được gỡ khỏi cả hai locale. Phần còn lại của gạch đầu dòng này — task Google OAuth từng thật sự không có đích, và #307 là ticket dựng đề mục cho nó — vẫn nguyên giá trị.
